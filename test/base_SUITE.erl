...
lru_deletion_disk_limit(_Config) ->
    ...
    [] = http_cache_store_disk:list_candidates(?TEST_REQUEST_KEY, ?TEST_OPTS),
    receive
        {[http_cache_store_disk, object_deleted], TelemetryRef, #{}, #{reason := lru_nuked_disk}} ->
...
lru_deletion_memory_limit(_Config) ->
    ...
    [] = http_cache_store_disk:list_candidates(?TEST_REQUEST_KEY, ?TEST_OPTS),
    receive
        {[http_cache_store_disk, object_deleted], TelemetryRef, #{}, #{reason := lru_nuked_memory}} ->
            telemetry:detach(TelemetryRef)
    after 1000 ->
        ct:fail(timeout_receive_telemetry_event)
    end.
...
invalidate_by_url(_Config) ->
    ...
    [] = http_cache_store_disk:list_candidates(?TEST_REQUEST_KEY, ?TEST_OPTS),
    receive
        {[http_cache_store_disk, object_deleted], TelemetryRef, #{}, #{reason := url_invalidation}} ->
            telemetry:detach(TelemetryRef)
    after 1000 ->
        ct:fail(timeout_receive_telemetry_event)
    end.
